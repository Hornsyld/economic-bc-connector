table 51009 "Economic Activities Cue"
{
    ...
    fields
    {
        ...
        field(2; CustomersToSync; Integer)
        {
            ...
            CalcFormula = count(Customer where("Economic Sync Status" = filter(Modified)));
        }
        field(3; VendorsToSync; Integer)
        {
            ...
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = count(Vendor where("Economic Sync Status" = filter(Modified)));
        }
        ...
        field(5; CustomersModified; Integer)
        {
            ...
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = count(Customer where(SystemModifiedAt = filter(>= LastDateTimeValue), "Economic Sync Status" = filter(New)));
        }
        field(6; VendorsModified; Integer)
        {
            ...
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = count(Vendor where(SystemModifiedAt = filter(>= LastDateTimeValue), "Economic Sync Status" = filter(New)));
        }
        field(7; IntegrationLogs; Integer)
        {
            ...
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = count("Economic Integration Log" where(SystemCreatedAt = filter(>= LastDateTimeValue)));
        }
